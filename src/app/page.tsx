...
export default async function SchoolSelectPage() {
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-xl flex-col items-center px-6 pt-32 text-center">
      ...
      <h1 className="mb-20 text-[1.8rem] font-medium leading-tight tracking-wide text-foreground">
        校園空間借用系統
      </h1>

      <div className="flex w-full flex-col gap-3">
        {schools.map((school) => (
          <Link key={school.id} href={`/${school.slug}`} className="muji-block-button block text-center">
            {school.name}
          </Link>
        ))}
        {schools.length === 0 && (
      ...
      <div className="mt-36 text-[0.7rem] uppercase tracking-wide text-muted-foreground">
